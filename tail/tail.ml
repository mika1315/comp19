...
(* Tail.g_expr : First.t -> string -> string list -> string *)
...
(* 末尾呼び出し最適化を行なうコード生成 *)
(* Tail.t_expr : First.t -> string *)
let rec t_expr expr counter = match expr with
  ...
  | IfEqual (name1, name2, expr3, expr4) ->
    let label1 = make_label counter in
    cmpq name1 name2 ^ (jne label1) ^
    t_expr expr3 counter ^ label label1 ^ t_expr expr4 counter
  | IfLess (name1, name2, expr3, expr4) ->
    let label1 = make_label counter in
    cmpq name1 name2 ^ (jle label1) ^
    t_expr expr3 counter ^ label label1 ^ t_expr expr4 counter
  | Let ((name, t), expr1, expr2) ->
    g_expr expr1 name (ValSet.remove name (free_v expr2)) counter ^ t_expr expr2 counter 
  | Application (name, register_list) -> jmp name
  
(* Tail.g_def : First.def_t -> string *)
let g_def (FunDef ((name, t), params, expr)) =
  label name ^ t_expr expr counter

(* Tail.g_program : First.prog_t -> string *)
